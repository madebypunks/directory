...
// Bot identity
const BOT_NAME = "TschuuulAI";
const BOT_AVATAR = "https://punks.art/api/traits/006-125-127-124-220?background=v2&format=png&size=240";
...
const REPO_OWNER = process.env.GITHUB_REPO_OWNER || "madebypunks";
const REPO_NAME = process.env.GITHUB_REPO_NAME || "directory";
...
async function getPRFiles(prNumber: number): Promise<PRFile[]> {
  ...
  for (const file of files) {
    // Only care about content files (punks and projects)
...
async function getPRDetails(prNumber: number) {
...
async function analyzeWithClaude(prDetails: any, files: PRFile[]): Promise<ReviewResult> {
...
function formatComment(result: ReviewResult): string {
  const lines: string[] = [
...
export async function GET() {
  ...
  for (const pr of prs) {
    ...
    const [details, files] = await Promise.all([getPRDetails(pr.number), getPRFiles(pr.number)]);

    // Skip if no content files changed (only code/config changes)
    if (files.length === 0) {
      skipped.push(pr.number);
      continue;
    }
    ...
    await postComment(pr.number, formatComment(result));
